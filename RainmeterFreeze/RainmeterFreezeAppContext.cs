...
public sealed class RainmeterFreezeAppContext : ApplicationContext
{
    ...
    /// </summary>
    public event Action<RainmeterFreezeAppContext> FreezeAlgorithmChanged;

    /// <summary>
    /// Occurs when the freeze mode of this <see cref="RainmeterFreezeAppContext"/> has changed.
    /// </summary>
    public event Action<RainmeterFreezeAppContext> FreezeModeChanged;
    ...
    /// </summary>
    private void HandleWindowChanged(nint hWinEventHook, uint eventType, nint hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
    {
        if (!RefreshRainmeterPid())
            return;
